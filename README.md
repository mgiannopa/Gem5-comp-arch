...
Στο αρχέιο stats.txt αναφέρεται ότι ο αριθμός των commited εντολών είναι 5027.  
`system.cpu_cluster.cpus.committedInsts           5027  `  
Το νούμερο αυτό διαφέρει από το τελικό νούμερο που παρουσίαζει ο gem5, καθώς δεν συμπεριλαμβάνει εντολές που εν τέλει απορρίφθηκαν όπως π.χ. από ένα εσφαλμένο branch prediction, έτσι ο συνολίκος αριθμός των εντολών που έγινε succesfully decoded είναι αρκετά μεγαλύτερος.  
### Πόσες φορές προσπελάστηκε η L2 cache?  
Ο συνολικός αριθμός προσπελάσεων (read+write) στην L2 είναι 474.  
...
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Simple CPU Models** | Το SimpleCPU είναι ένα λειτουργικό μοντέλο in-order που προορίζεται για περιπτώσεις όπου δεν απαιτείται λεπτομερές μοντέλο.                                                                                                                        |
...
| AtomicSimpleCPU     | Χρησιμοποιεί ατομικές προσπελάσεις μνήμης. Είναι κληρονομημένη από την BaseSimpleCPU και περιέχει συναρτήσεις tick, οι οποίες ορίζουν τι συμβαίνει σε κάθε κύκλο του επεξεργαστή.                                                                    |
| TimingSimpleCPU     | Χρησιμοποιεί χρονισμένες προσπελάσεις μνήμης. Καθυστερεί στις προσπελάσεις cache και περιμένει για την απάντηση από το σύστημα μνήμης πριν προχωρήσει.                                                                                                |
| **Minor CPU Model**  | MinorCPU είναι ένα μοντέλο επεξεργαστή με in-order εκτέλεση που διαθέτει pipeline και παραμετροποιήσιμες δομές και συμπεριφορές. Κατάλληλο για την ανάλυση και συσχέτιση μικρο-αρχιτεκτονικών χαρακτηριστικών επεξεργαστών.                            |
| MinorCPU            | Η βασική λειτουργία του περιλαμβάνει τη μοντελοποίηση ενός επεξεργαστή με αυστηρή in-order εκτέλεση και τη δυνατότητα οπτικοποίησης των εντολών στην pipeline. Η σχεδίασή του βασίζεται στο multithreading και data structures. |  
...
Αρχικά ορίζοντας μόνο τον τύπο του Cpu για MinorCPU έχουμε 0.00041 sim_seconds και το host_seconds 0.06 σύμφωνα με το stats.txt. Η εντολή που τρέξαμε έιναι :  
...
![image](https://github.com/user-attachments/assets/22da30c9-44c0-42c6-9ed2-88fe0327b41f)   
Ομοίως αλλάζοντας τον τύπο της Cpu σε TimingSimpleCPU παίρνουμε sim_seconds 0.000051 και host_seconds 0.02.  
![image](https://github.com/user-attachments/assets/96d78000-2543-46e9-8b0c-c6c4417de057)  
...
![image](https://github.com/user-attachments/assets/22c95392-e580-403a-9a04-37d9107d9c4b)  
Για τον TimingSimpleCPU έχουμε sim_seconds 0.000527 και host_seconds 0.03.  
![image](https://github.com/user-attachments/assets/3bb1e3e2-f298-4bb1-88d3-2db4bd852912)  
...
![image](https://github.com/user-attachments/assets/3a771a30-d3a1-4605-b94d-9cd8a30f0f15)  
Για TimingSimpleCPU έχουμε sim_seconds 0.000051 και host_seconds 0.02.   
![image](https://github.com/user-attachments/assets/85a06d27-adf3-4871-aabf-f72348956c89)  
...
Σίγουρα ένας συνδυασμός της αύξησης τόσο της συχνότητας λειτουργίας όσο και της ταχύτητας της μνήμης θα έχει τα βέλτιστα αποτελέσματα. Στα διαγράμματα φαίνεται εύκολα η τεράστια διαφορά τάξης μεγέθους μεταξύ των 2 χρόνων.   
## Δεύτερο Μέρος :  
...
Παρατηρήθηκε μια πολύ μικρή μείωση του χρόνου από  0.083982 σε 0.083609.  
![image](https://github.com/user-attachments/assets/c142ea97-92cb-4709-9ef5-7e9aad9fdfbc)  
Η αλλαγή αφορούσε το πρώτο benchmark(specbzip). Τα αρχεία που παράχθηκαν με την αλλαγή της μνήμης βρίσκονται εντός του φακέλου specbzip2133.  
### Design Exploration  
#### Specbzip  
...
![image](https://github.com/user-attachments/assets/70ca4ed6-9402-4be3-8058-b5f8d3fdf5dc)  
#### Specmcf  
...
![image](https://github.com/user-attachments/assets/ce5c5978-95d4-4e61-8b66-a37c184494e0)  
...
![image](https://github.com/user-attachments/assets/a9dc7fe9-9091-4cef-bb49-667ec344543f)  
#### Specsjeng  
...
με cpi = 3.072115.  
![image](https://github.com/user-attachments/assets/b5331fc4-6d51-4d63-836f-77586774eb24)  
#### Speclibm  
...
![image](https://github.com/user-attachments/assets/afc5c83e-8564-4dc0-ab34-0f64ace43ea9)  

Σε όλα τα γραφήματα που παρουσιαάστηκαν στην συγκεκριμένη υποενότητα βλέπουμε από αριστερά προς τα δεξιά στο γράφημα το αρχικό με το τελικό cpi σύμφωνα με τις αλλαγές που αναφέρθηκαν παραπάνω.  
### Συνάρτηση κόστους  
Έστω C=0.7* (S1+S2)*10<sup>-3</sup> + 0.3 * S3 + 0.5(A1 + A2) + 0.2 * A3 + 0.3 L  
